...
export function HeroSection() {
  ...
  const textBoxConfig = {
    ...
    leftOffset: -100,
    widthMultiplier: 0,
    nameSize: "text-xl md:text-2xl", // smaller main text
    subtitleSize: "text-sm md:text-base", // smaller subtitle
    padding: "px-4 py-2", // reduced padding
  ...
  const getTextBoxPosition = () => {
    const positions: Record<string, any> = {
      top: { top: `${textBoxConfig.offset}rem` },
      center: { top: "50%", transform: "translateY(-50%)" },
      bottom: { bottom: `${textBoxConfig.offset}rem` },
    }
    if (positions[textBoxConfig.position]) return positions[textBoxConfig.position]
    return { top: textBoxConfig.position }
  }

  const textBoxPositionStyle = getTextBoxPosition()
    ...
    transform:
      showTilt && isHovering
        ? `perspective(1000px) rotateY(${mousePosition.x * 18}deg) rotateX(${
            -mousePosition.y * 18
          }deg) translateZ(80px) translateX(${
            mousePosition.x * 15 + (textBoxConfig.leftOffset || 0)
          }px) translateY(${mousePosition.y * 15}px)`
        : `perspective(1000px) rotateY(0) rotateX(0) translateZ(0) translateX(${
            textBoxConfig.leftOffset || 0
          }px) translateY(0)`,
        ...
        <div className="max-w-2xl mx-auto text-center">
          ...
          >
            ...
            >
              ...
              {/* Floating 3D Text Layer - Smaller */}
              <div
                className="parallax-text-overlay absolute"
                style={{
                  ...textParallaxStyle,
              ...
              >
                <div className="relative inline-block px-4 py-2 rounded-md shadow-md">
                  <Image
                    src="/layer 2.webp"
                    alt="text background"
                    fill
                  ...
                  <div className="relative z-10 text-center">
                    <h2
                      className={`${textBoxConfig.nameSize} font-semibold text-[#5d4e37] leading-tight whitespace-nowrap`}
                    >
                      Aysha Salma
                    </h2>
                    <p
                      className={`${textBoxConfig.subtitleSize} text-[#6b5b46] mt-1 font-medium whitespace-nowrap italic`}
                    >
